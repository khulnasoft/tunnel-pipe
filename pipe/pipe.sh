...
echo "Running tunnel with options: ${ARGS}" "${artifactRef}"
echo "Global options: " "${GLOBAL_ARGS}"
tunnel $GLOBAL_ARGS ${scanType} $ARGS ${artifactRef} 
returnCode=$?
...
if [[ ${template} == *"sarif"* ]]; then
  ...
  tunnel --quiet ${scanType} --format template --template ${template} --output ${output} $SARIF_ARGS ${artifactRef} $BITBUCKET_CLONE_DIR
fi
